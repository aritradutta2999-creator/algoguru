...
public class Knapsack {
  ...
  },
  {
    ...
    spaceComplexity: "O(n)",
    theory: [
      "Longest Increasing Subsequence (LIS): Find the longest subsequence of a given sequence such that all elements are in strictly increasing order. Example: for [10, 9, 2, 5, 3, 7, 101, 18], the LIS is [2, 3, 7, 18] with length 4.",
      "O(n²) DP approach: dp[i] = length of LIS ending at index i. For each i, check ALL j < i: if nums[j] < nums[i], then dp[i] = max(dp[i], dp[j] + 1). This is intuitive but too slow for large inputs (n > 10⁴).",
      "O(n log n) approach using patience sorting: Maintain a 'tails' array where tails[k] = smallest possible tail element of all increasing subsequences of length k+1. For each new element, binary search for its position in tails.",
      "Intuition for O(n log n): Imagine you're playing patience (solitaire). Place each card on the leftmost pile whose top card is ≥ the new card. If no such pile exists, start a new pile. The number of piles = LIS length!",
      "The binary search approach doesn't directly give the actual LIS — it gives the length. To reconstruct the actual LIS, use the O(n²) approach with a parent array, or augment the O(n log n) approach with additional tracking.",
      "LIS variants are extremely popular in competitive programming: Longest Non-Decreasing Subsequence (use ≤ instead of <), Number of LIS (track count alongside length), Longest Bitonic Subsequence (increases then decreases), Russian Doll Envelopes (2D LIS).",
    ],
    keyPoints: [
      "O(n²): dp[i] = max(dp[j] + 1) for all j < i where nums[j] < nums[i]",
      "O(n log n): maintain tails array, binary search for each element",
      "tails array is always sorted — enables binary search",
    ...
    tip: "In competitive programming, always use the O(n log n) approach for LIS. Use Arrays.binarySearch or manual binary search. For the actual subsequence, use the O(n²) approach with parent tracking — it's rarely needed in contests though.",
    code: [
      {
