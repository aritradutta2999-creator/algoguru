...
// Safe for graphs with millions of vertices`
  ...
  },
  {
    ...
    theory: [
      "Dijkstra finds shortest paths from a single source to all other vertices in a weighted graph with NON-NEGATIVE edge weights. It's the most important shortest path algorithm in competitive programming. Named after Edsger W. Dijkstra who published it in 1959.",
      "Core idea (greedy): Maintain array d[] where d[v] = current shortest distance from source s. Initially d[s]=0, all others ∞. Also maintain a boolean array u[] (is vertex finalized?). At each iteration, pick the unfinalized vertex v with smallest d[v], finalize it, and relax all edges (v, to): d[to] = min(d[to], d[v] + weight(v, to)).",
      "**Correctness proof (by induction)**: After vertex v is finalized, d[v] is optimal. Consider the shortest path P to v, split into finalized part P1 and unfinalized part P2 starting at vertex p. Since p was relaxed from a finalized vertex, d[p] = optimal. Since all weights ≥ 0: d[p] ≤ d[v]. But v was chosen as the minimum unfinalized vertex, so d[v] ≤ d[p]. Therefore d[v] = optimal distance.",
      ...
      "**Two implementations**: (1) **O(n² + m) — for dense graphs**: scan all vertices to find minimum, no priority queue needed. Optimal when m ≈ n². (2) **O((n + m) log n) — for sparse graphs**: use min-heap (priority queue). The runtime splits into n extractions of minimum (n log n) and m relaxations (m log n).",
      "**Path reconstruction**: Maintain predecessor array p[] where p[to] = v whenever d[to] is improved through v. The shortest path to vertex v is: v → p[v] → p[p[v]] → ... → s, then reverse.",
      ...
      "**Optimization**: The algorithm can stop early as soon as the selected vertex has d[v] = ∞ (all remaining vertices are unreachable). If you only need the distance to a single target t, stop when t is popped from the PQ.",
    ],
    keyPoints: [
...
public class Dijkstra {
  ...
  },
  {
    ...
    theory: [
      "**Bellman-Ford** handles negative edge weights and detects negative cycles. Named after Richard Bellman (1958) and Lester Ford (1956). It works by relaxing ALL edges V-1 times. Why V-1? Because the shortest path between any two vertices has at most V-1 edges.",
      ...
      "**Early termination optimization**: Keep a flag that tracks whether any relaxation occurred in the current phase. If no relaxation happens in a phase, the algorithm can stop early — all distances are already optimal. This doesn't improve worst-case but significantly speeds up average case on random graphs.",
      "**Important guard**: When relaxing, check `if (d[u] < INF)` before computing `d[u] + w`. Without this, negative weight edges can produce incorrect distances like ∞-1, ∞-2 etc. for unreachable vertices.",
      "Negative cycle detection: After V-1 relaxation rounds, do one more round. If ANY edge can still be relaxed, a negative cycle exists. To **find the actual cycle**: track predecessors, find any vertex x that was relaxed in the V-th phase, then follow predecessors V times from x to ensure you're inside the cycle, then trace the cycle.",
      "When to use Bellman-Ford vs Dijkstra: Use Bellman-Ford when edges can have negative weights, or when you need to detect negative cycles. Use Dijkstra when all weights are non-negative (it's faster: O((V+E)logV) vs O(VE)). Note: Bellman-Ford is simpler to implement with edge lists.",
      "**Floyd-Warshall** finds ALL-PAIRS shortest paths in O(V³). Named after Robert Floyd and Stephen Warshall (1962), though Bernard Roy published the same algorithm in 1959. It answers: 'What is the shortest distance between EVERY pair of vertices?'",
      "Floyd-Warshall DP insight: Before phase k, d[i][j] = shortest path from i to j using only vertices {0,...,k-1} as intermediates. At phase k: d[i][j] = min(d[i][j], d[i][k] + d[k][j]). **No separate matrix needed** — updates can be done in-place because improving through k cannot worsen future results.",
      "**Path reconstruction in Floyd-Warshall**: Maintain matrix p[i][j] storing the last intermediate vertex k that improved d[i][j]. To reconstruct i→j path: recursively find path(i, p[i][j]) and path(p[i][j], j).",
      "Floyd-Warshall handles negative weights but NOT negative cycles. To detect negative cycles: check if d[i][i] < 0 for any vertex i after the algorithm completes. **Floating-point warning**: errors accumulate as O(2^n · δ) in the worst case — use integer arithmetic when possible.",
    ],
    keyPoints: [
...
public class AllPairsShortestPath {
  ...
  },
  {
    ...
    theory: [
      ...
      "**Kruskal's Algorithm** (Joseph Kruskal, 1956): Sort all edges by weight. Iterate in ascending order. For each edge, if it connects two different components (checked via Union-Find), add it to the MST. The simple implementation is O(M log M + N²), but with DSU using path compression + union by rank, it becomes O(M log M + M·α(N)) ≈ O(M log M).",
      "**Kruskal's correctness proof**: By induction. At any stage, the chosen edge set F is a subset of some MST T. When we add edge e: if e ∈ T, done. If e ∉ T, adding e to T creates a cycle containing another edge e' that crosses the same cut as e. Since e was considered first (sorted), w(e) ≤ w(e'), so replacing e' with e gives another MST containing F ∪ {e}.",
      ...
      "MST applications: Network design (minimum cost to connect all cities), clustering (remove k-1 heaviest edges to get k clusters), **second-best MST** (replace each MST edge with the best non-MST edge crossing the same cut, take the minimum increase), bottleneck spanning tree.",
    ],
    keyPoints: [
...
public class MinimumSpanningTree {
  ...
  },
  {
    ...
    theory: [
      ...
      "**DFS-based approach** (from cp-algorithms): Run DFS, append vertex v to a list when dfs(v) finishes (post-order). For every directed edge v→u, u finishes before v (since u is reachable from v and thus visited during/before dfs(v)). Reversing this list gives topological order. The exit time of v is always greater than exit times of all vertices reachable from v.",
      ...
      "Classic applications: Course scheduling (prerequisites), build systems (Makefile), task scheduling, compilation order, dependency resolution in package managers. Also: shortest paths in DAGs (relax edges in topological order — O(V+E)).",
      ...
      "**Condensation graph**: Replace each SCC with a single node. The result is always a **DAG** (acyclic by definition — if two SCCs had a cycle between them, they'd be one SCC). This lets you solve problems on cyclic directed graphs by reducing to DAG problems.",
